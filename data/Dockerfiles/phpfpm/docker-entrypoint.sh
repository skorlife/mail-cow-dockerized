...
# only take care of this when using integrated mailcow db container and not a remote one
if [[ "${DBCONNECTION}" == "unix" ]]; then
  CONTAINER_ID=
...
TZ_CHECK=$(mysql ${DBCONNECTION_STR} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT CONVERT_TZ('2019-11-02 23:33:00','Europe/Berlin','UTC') AS time;" -BN 2> /dev/null)
...
if [[ "${MASTER}" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
  ...
  do
    DOMAIN_ARR+=("$line")
  done < <(mysql ${DBCONNECTION_STR} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT domain FROM domain" -Bs)
  while read line
  do
    DOMAIN_ARR+=("$line")
  done < <(mysql ${DBCONNECTION_STR} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SELECT alias_domain FROM alias_domain" -Bs)
