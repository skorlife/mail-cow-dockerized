...
try:
  ...
  def query_mysql(query, headers = True, update = False):
    while True:
      try:
        if (not os.environ.get('DBCONNECTION') or os.environ.get('DBCONNECTION') == 'unix'):
          ...
          cnx = mysql.connector.connect(host = os.environ.get('DBCONNECTION'), user=os.environ.get('DBUSER'), passwd=os.environ.get('DBPASS'), database=os.environ.get('DBNAME'), charset="utf8mb4", collation="utf8mb4_general_ci")
      except Exception as ex:
        print('%s - trying again...'  % (ex))
        time.sleep(3)
      else:
