...
if [[ -z "${DBCONNECTION}" || "${DBCONNECTION}" == "unix" ]]; then
    ...
    DBCONNECTION_STR_CF="host=/var/run/mysqld/mysqld.sock"
    DBCONNECTION_STR_LUA="localhost"
else
    DBCONNECTION_STR="--host=${DBCONNECTION}"
    DBCONNECTION_STR_CF="host=${DBCONNECTION}"
    DBCONNECTION_STR_LUA="${DBCONNECTION}"
fi
...
connect = "${DBCONNECTION_STR_CF} dbname=${DBNAME} user=${DBUSER} password=${DBPASS}"
map {
...
function script_init()
  ...
  ltn12 = require "ltn12"
  env  = mysql.mysql()
  con = env:connect("__DBNAME__","__DBUSER__","__DBPASS__","__DBCONNECTION__")
  return 0
end
...
sed -i "s/__DBCONNECTION__/${DBCONNECTION_STR_LUA}/g" /etc/dovecot/lua/passwd-verify.lua
sed -i "s/__IPV4_SOGO__/${SOGO_IP}/g" /etc/dovecot/lua/passwd-verify.lua
...
passdb {
  driver = static
  args = allow_real_nets=${SOGO_IP}/32 password={plain}${RAND_PASS}
}
EOF
...
IMAPSYNC_TABLE=$(mysql ${DBCONNECTION_STR} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "SHOW TABLES LIKE 'imapsync'" -Bs)
[[ ! -z ${IMAPSYNC_TABLE} ]] && mysql ${DBCONNECTION_STR} -u ${DBUSER} -p${DBPASS} ${DBNAME} -e "UPDATE imapsync SET is_running='0'"
...
while [[ ${VERSIONS_OK} != 'OK' ]]; do
  if [[ ! -z $(mysql ${DBCONNECTION_STR} -u ${DBUSER} -p${DBPASS} ${DBNAME} -B -e "SELECT 'OK' FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = \"${DBNAME}\" AND TABLE_NAME = 'versions'") ]]; then
    VERSIONS_OK=OK
  else
